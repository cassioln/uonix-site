...
- `ENABLE_DEPLOY_DEVELOPMENT=true` é **variável órfã**: o `deploy-development.yml` foi removido junto com o ambiente DEV, e nenhum workflow a lê. Pode ser apagada. Já `DEVELOPMENT_URL` e `HOSTGATOR_DEV_ROOT` ainda têm consumidor em `clone-environment.yml` e `scripts/lib/environment-map.sh`, então a remoção do DEV segue em andamento.
