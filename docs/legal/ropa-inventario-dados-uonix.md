...
| **09** | **Consultas de Busca do Search Console**<br>(`53-admin-analytics-metrics.php` → `wp_options`) | Texto livre da consulta digitada no Google, com as métricas associadas (impressões, cliques, posição, CTR). **Não** há identificador de sessão, IP ou cookie. *(Restrição técnica: o filtro `uonix_analytics_metrics_sanitize_query()` descarta a consulta que contenha e-mail em formato canônico, sequência de 8+ dígitos, URL ou texto acima de 120 caracteres — mas **não** remove nome próprio, CPF formatado com pontuação, CEP de 5 dígitos nem placa de veículo. A consulta é texto digitado por terceiro e pode conter identificador pessoal residual.)* | Visitantes que chegaram ao site por busca orgânica no Google. Titular **não identificado nem identificável pela Uônix**: o Google entrega a consulta agregada, sem vínculo com pessoa. | Mineração de oportunidades de SEO: identificar consultas em posição 4 a 12 com baixo CTR para priorizar ajuste de conteúdo técnico (Central de Inteligência, Módulo 3). | **Art. 7º, IX** (Legítimo Interesse do controlador em otimizar a descoberta do próprio conteúdo técnico, sem tratamento posterior incompatível). Base sujeita ao teste de proporcionalidade do Art. 10 e à minimização do Art. 6º, III. | **Justificativa para guarda, sem prazo fixo:** o dado é retido enquanto o Módulo 3 operar, porque a única cópia é o *snapshot* corrente do período, que é **sobrescrito** a cada sincronização — não há acúmulo histórico. Gerações anteriores são eliminadas automaticamente quando se tornam inalcançáveis. O texto da consulta é descartado para as linhas que não podem constituir oportunidade: do universo medido de 111 consultas em 30 dias, o texto de ~106 não é persistido. O residual é de **10 linhas incondicionais** (lista curta exibida no painel) mais as que estão na faixa de retenção. Eliminação integral ocorre ao desativar o módulo ou por `delete_option`. | Nenhum compartilhamento. O dado **entra** do Google Search Console (API, conta de serviço) e não sai: permanece no banco MySQL de produção e no backup de cada publicação. O relatório executivo semanal é enviado apenas ao operador interno. **Não** atravessa ambientes: `scripts/clone-environment.sh` protege a opção, então um clone para QA ou máquina local não a herda. | Acesso ao painel exige capability `edit_posts`; gravação exige `manage_options` e nonce. Filtro de PII na normalização, antes da persistência. Opção gravada com `autoload = false`. Snapshot fora do alcance de clone entre ambientes, com guarda de regressão em `test-clone-non-inheritable-options.sh`. Credencial da API em arquivo fora do document root, referenciado por constante no `wp-config.php`. |
