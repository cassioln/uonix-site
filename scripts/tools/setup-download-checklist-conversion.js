...
async function main() {
  ...
  if (!existingTrigger) {
    ...
    if (evtParam !== 'uonix_download_checklist') trDivergent = true;
  ...
  if (!existingTag) {
    ...
    const hasLabel = (existingTag.parameter || []).some(p => p.key === 'conversionLabel' && p.value === '{{Constante - Label Download Checklist}}');
