...
# O ambiente remoto de desenvolvimento saiu da topologia, e o mapa é o primeiro
# passo de toda a tooling. A asserção é INVERTIDA de propósito: o nome retirado
# tem de ser REJEITADO, não silenciosamente resolvido. Sem isto, reintroduzir o
# ambiente passaria verde, e o clone voltaria a aceitar um destino cujo docroot e
# URL já não estão declarados — apontando rsync para caminho vazio.
for retirado in dev development; do
  if uonix_env_canonical "$retirado" >/dev/null 2>&1; then
    fail "ambiente retirado <$retirado> voltou a ser aceito pelo mapa"
  fi
done
...
printf 'PASS: mapa shell cobre produção, QA e local, rejeita o ambiente retirado e não tem defaults silenciosos.\n'
