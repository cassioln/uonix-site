...
for needle, message in (
    ...
    ('global $wp_cache_slash_check, $wp_cache_mod_rewrite, $wp_cache_config_file;', 'prova fresca não traz campos omitidos pelo bootstrap WP-CLI para o escopo global'),
    ('is_link($wp_cache_config_file)', 'prova fresca aceita config por symlink'),
    ('realpath($wp_cache_config_file) !== realpath($expected_config)', 'prova fresca não confina o include ao config WPSC esperado'),
    ('include $wp_cache_config_file;', 'prova fresca não relê fisicamente o config WPSC'),
